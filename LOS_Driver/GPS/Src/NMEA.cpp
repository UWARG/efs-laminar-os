...
int decodeGGA (char *GGAbuffer, GGASTRUCT *gga)
{
	...
	if (GGAbuffer[inx] == '1')   // 0 indicates no fix yet
	{
		gga->isfixValid = 1;   // fix available
		inx = 0;   // reset the index. We will start from the inx=0 and extract information now
	}
	else if (GGAbuffer[inx] == '2')
	{
		gga->isfixValid = 2;   // Differential GNSS fix,
		inx = 0;   // reset the index. We will start from the inx=0 and extract information now
	}
	else if ((GGAbuffer[inx] == '6'))
	{
		gga->isfixValid = 3;   //Estimated/Dead reckoning fix
		inx = 0;   // reset the index. We will start from the inx=0 and extract information now
	}
	else
	{
...
//(Update the GMT Offset at the top of this file)
	...
	gga->tim.allinsec = atoi(buffer)%100 + min* 60 + hr * 60;
