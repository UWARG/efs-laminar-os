...
uint32_t PPMChannel::get_us(uint8_t num)
{
...
{
	if(channel >= num_channels || channel >= MAX_PPM_CHANNELS) {
		return 0;
	}

	uint8_t percentage = time_to_percentage(max_values[channel], min_values[channel], ppm_values[channel]);
	return percentage;
}
