#include "PPM.hpp"
#include "tim.h"
#include "RSSI.hpp"// gotta figure out where to put all of this later.
...
static float counterToTime(uint32_t count, uint32_t psc);
static uint8_t timeToPercentage(uint32_t max, uint32_t min, float time);
...
uint8_t PPMChannel::GetResult(uint8_t channel)
{
	...
	uint8_t percentage = timeToPercentage(max_values_[channel], min_values_[channel], ppm_values[channel]);
	return percentage;
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	if (htim->Instance == PPM_TIME_BASE)
	{

		static volatile uint8_t index = 0;

		volatile uint32_t time_diff = HAL_TIM_ReadCapturedValue(htim, PPM_TIM_CHANNEL);
		__HAL_TIM_SET_COUNTER(htim, 0);

		float pulseLength = counterToTime(time_diff, htim->Init.Prescaler) - PULSE_WIDTH;
