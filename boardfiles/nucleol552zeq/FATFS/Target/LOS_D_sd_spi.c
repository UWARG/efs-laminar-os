...
{
	while(!__HAL_SPI_GET_FLAG(LOS_D_SD_SPI, SPI_FLAG_TXE));
	HAL_SPI_Transmit(LOS_D_SD_SPI, &data, 1, SPI_TIMEOUT);
}
...
{
	while(!__HAL_SPI_GET_FLAG(LOS_D_SD_SPI, SPI_FLAG_TXE));
	HAL_SPI_Transmit(LOS_D_SD_SPI, buffer, len, SPI_TIMEOUT);
}
...
static uint8_t SPI_RxByte(void)
{
	...
	while(!__HAL_SPI_GET_FLAG(LOS_D_SD_SPI, SPI_FLAG_TXE));
	HAL_SPI_TransmitReceive(LOS_D_SD_SPI, &dummy, &data, 1, SPI_TIMEOUT);
